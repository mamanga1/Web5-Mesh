...
//
// prologue: Datos opcionales de vinculación de identidad (e.g., DIDs)
// para atar criptográficamente la identidad de capa superior a la sesión Noise.
func NewHandshakeIK(isInitiator bool, myPrivX *[32]byte, myPubX *[32]byte, theirPubX *[32]byte, prologue []byte) (*HandshakeState, error) {
...
// BuildNoisePrologue genera un buffer de prólogo estándar vinculando los DIDs
// de ambos extremos para garantizar el binding DID <-> X25519 en el handshake Noise.
func BuildNoisePrologue(initiatorDID, responderDID string) []byte {
return []byte("web5-mesh:v1:" + initiatorDID + "->" + responderDID)
}
