...
const (
// MaxHandshakeSizeBytes evita ataques de buffer/deserialización masivos (DoS).
MaxHandshakeSizeBytes = 1024
// MaxHandshakesPerMinute limita los intentos por IP para evitar saturar la CPU con ed25519.Verify.
...
// 1. DESCARTE EN O(1) POR TAMAÑO DE BUFFER (Filtro DoS instantáneo sin allocs pesados)
if len(data) == 0 || len(data) > MaxHandshakeSizeBytes {
return "", fmt.Errorf("payload de tamaño inválido")
}

// 2. RATE LIMITER POR IP (Evita agotar la CPU con ed25519.Verify bajo inundación UDP)
ip := extractIP(remoteAddr)
if !g.allowRateLimit(ip) {
return "", fmt.Errorf("rate limit excedido")
}
...
func (g *Gate) allowRateLimit(ip string) bool {
g.mu.Lock()
defer g.mu.Unlock()

now := time.Now()
entry, ok := g.rateLimits[ip]
if !ok || now.Sub(entry.lastCheck) > time.Minute {
g.rateLimits[ip] = &rateLimitEntry{
tokens:    MaxHandshakesPerMinute - 1,
lastCheck: now,
}
return true
}
...
for ip, entry := range g.rateLimits {
if now.Sub(entry.lastCheck) > 5*time.Minute {
delete(g.rateLimits, ip)
}
}
}
