...
// FIX 19: respetar $XION_HOME para ubicar acl.json.
// Si XION_HOME está definido, usa $XION_HOME/acl.json.
// Si no, usa ./acl.json (comportamiento anterior).
var ACLFile = func() string {
if home := os.Getenv("XION_HOME"); home != "" {
return filepath.Join(home, "acl.json")
}
return "acl.json"
}()
...
// GetByPubKeyX busca un peer en la ACL utilizando su clave pública X25519 (hex).
func (a *ACL) GetByPubKeyX(pubKeyXHex string) (PeerInfo, bool) {
for _, peer := range a.Peers {
if peer.PubKeyX == pubKeyXHex {
return peer, true
}
}
...
// ValidateBinding valida si un DID está asociado exactamente a la clave pública X25519 provista.
func (a *ACL) ValidateBinding(did string, pubKeyX []byte) bool {
peer, ok := a.Peers[did]
if !ok {
return false
}
return peer.PubKeyX == hex.EncodeToString(pubKeyX)
}
